// ============================================================
// ASI filename-driven Prince of Persia (2008) universal helper v28 stable
...
//
// Build as a 32-bit DLL/ASI. Generated filenames and log identity follow the actual ASI filename.
// ============================================================
#include <windows.h>
#include <stdio.h>
#include <string.h>
...
// Runtime identity is derived from the actual ASI module filename.
// Example: MyPatch.asi -> MyPatch.ini, MyPatch.log,
// MyPatch_CompatibilityReport.txt and log prefix [MyPatch].
static wchar_t g_asiPath[MAX_PATH * 4] = {0};
static wchar_t g_asiFileName[MAX_PATH * 4] = {0};
static wchar_t g_asiBaseName[MAX_PATH * 4] = {0};
static char g_logPrefixUtf8[MAX_PATH * 4] = {0};
...
static void LogMessageV(int level, const char* fmt, va_list ap)
{
    ...
    _snprintf_s(line, sizeof(line), _TRUNCATE, "%s%s",
                g_logPrefixUtf8[0] ? g_logPrefixUtf8 : "[ASI] ", message);
#else
...
static void InitializeAsiIdentityAndOutputPaths(HMODULE module)
{
    g_asiPath[0] = 0;
    g_asiFileName[0] = 0;
    g_asiBaseName[0] = 0;
    ...
    if (!GetModuleFileNameW(module, g_asiPath, MAX_PATH * 4))
        lstrcpynW(g_asiPath, L"PoP_UniversalPatch.asi", MAX_PATH * 4);
    g_asiPath[MAX_PATH * 4 - 1] = 0;

    lstrcpynW(g_asiDir, g_asiPath, MAX_PATH * 4);
    wchar_t* lastSlash = wcsrchr(g_asiDir, L'\\');
    const wchar_t* fileName = lastSlash ? lastSlash + 1 : g_asiPath;
    if (lastSlash) *lastSlash = 0;
    else lstrcpynW(g_asiDir, L".", MAX_PATH * 4);

    lstrcpynW(g_asiFileName, fileName, MAX_PATH * 4);
    lstrcpynW(g_asiBaseName, fileName, MAX_PATH * 4);
    wchar_t* extension = wcsrchr(g_asiBaseName, L'.');
    if (extension && extension != g_asiBaseName) *extension = 0;
    if (!g_asiBaseName[0]) lstrcpynW(g_asiBaseName, L"PoP_UniversalPatch", MAX_PATH * 4);

#if defined(_MSC_VER)
    _snwprintf_s(g_iniPath, MAX_PATH * 4, _TRUNCATE, L"%ls\\%ls.ini",
                 g_asiDir, g_asiBaseName);
    _snwprintf_s(g_logPath, MAX_PATH * 4, _TRUNCATE, L"%ls\\%ls.log",
                 g_asiDir, g_asiBaseName);
    _snwprintf_s(g_reportPath, MAX_PATH * 4, _TRUNCATE, L"%ls\\%ls_CompatibilityReport.txt",
                 g_asiDir, g_asiBaseName);
#else
    swprintf(g_iniPath, MAX_PATH * 4, L"%ls\\%ls.ini", g_asiDir, g_asiBaseName);
    swprintf(g_logPath, MAX_PATH * 4, L"%ls\\%ls.log", g_asiDir, g_asiBaseName);
    swprintf(g_reportPath, MAX_PATH * 4, L"%ls\\%ls_CompatibilityReport.txt",
             g_asiDir, g_asiBaseName);
#endif

    char baseUtf8[MAX_PATH * 4] = {0};
    WideToUtf8(g_asiBaseName, baseUtf8, sizeof(baseUtf8));
#if defined(_MSC_VER)
    _snprintf_s(g_logPrefixUtf8, sizeof(g_logPrefixUtf8), _TRUNCATE, "[%s] ",
                baseUtf8[0] ? baseUtf8 : "ASI");
#else
    snprintf(g_logPrefixUtf8, sizeof(g_logPrefixUtf8), "[%s] ",
             baseUtf8[0] ? baseUtf8 : "ASI");
#endif
    g_logPrefixUtf8[sizeof(g_logPrefixUtf8) - 1] = 0;
}
...
static void ResolveVoiceLanguageAndFallback()
{
    ...
    if (VoicePackExists(requested)) {
        LogLine("Voice pack detected: requested language=%d", requested);
        return;
    }

    if (autoFallback && fallback != requested && VoicePackExists(fallback)) {
        g_effectiveVoiceLanguage = fallback;
        LogWarn("Requested voice pack language=%d is missing; safely falling back to language=%d", requested, fallback);
        return;
    }
    ...
    if (autoFallback)
        LogWarn("Requested voice pack language=%d is missing and fallback language=%d is unavailable; voice patches disabled", requested, fallback);
    else
        LogWarn("Requested voice pack language=%d is missing and AutoFallback=0; voice patches disabled", requested);
}
...
// ---- Registry values intercepted from the ASI-named INI file ----
                               ...
                               DWORD dwFlagsAndAttributes, HANDLE hTemplateFile)
{
    ...
    if (g_logLevel >= 3 && ShouldTraceFileA(lpFileName)) {
        LogLine("CreateFileA %s -> %s", lpFileName ? lpFileName : "(null)",
...
static bool PatchVTableEntry(void** vtbl, int index, void* replacement, void** realSlot, const char* label)
{
    ...
    if (realSlot && *realSlot && cur != *realSlot) {
        ...
        if (InterlockedCompareExchange(&s_loggedConflict, 1, 0) == 0)
            LogLine("%s vtable slot already hooked by another module; skipped", label ? label : "DirectInput");
        return false;
    }
...
HRESULT STDMETHODCALLTYPE Hook_DIDevice_SetCooperativeLevel(void* self, HWND hwnd, DWORD flags)
{
    if (ShouldAllowWinKeyInWindowed() && (flags & DISCL_NOWINKEY_LOCAL)) {
        ...
        if (InterlockedCompareExchange(&s_logged, 1, 0) == 0)
            LogLine("DirectInput DISCL_NOWINKEY cleared in windowed/borderless mode");
    }
    ...
    if (SUCCEEDED(hr) && outDevice && *outDevice && ShouldAllowWinKeyInWindowed()) {
        ...
        if (PatchVTableEntry(devVtbl, 13, (void*)Hook_DIDevice_SetCooperativeLevel,
            ...
            if (InterlockedCompareExchange(&g_diSetCoopHooked, 1, 0) == 0)
                LogLine("DirectInput device SetCooperativeLevel vtable hooked for Win-key fix");
        }
    }
    return hr;
}
...
static void TryPatchDirectInput8Object(void* directInputObj)
{
    ...
    if (PatchVTableEntry(vtbl, 3, (void*)Hook_DI8_CreateDevice,
        ...
        if (InterlockedCompareExchange(&g_diCreateDeviceHooked, 1, 0) == 0)
            LogLine("DirectInput8 CreateDevice vtable hooked for Win-key fix");
    }
}
    ...
    if (SUCCEEDED(hr) && out && *out && ShouldAllowWinKeyInWindowed()) {
        ...
        if (!GuidEqualLocal(riid, kIidDirectInput8ALocal)) {
            ...
            if (InterlockedCompareExchange(&s_loggedNonA, 1, 0) == 0)
                LogLine("DirectInput8Create returned non-A IID; attempting vtable hook anyway");
        }
        TryPatchDirectInput8Object(*out);
        static volatile LONG s_logged = 0;
        if (InterlockedCompareExchange(&s_logged, 1, 0) == 0)
            LogLine("DirectInput8Create intercepted without hard dinput dependency");
    }
    return hr;
}
...
FARPROC WINAPI Hook_GetProcAddress(HMODULE hModule, LPCSTR lpProcName)
{
    ...
    if (lstrcmpiA(lpProcName, "DirectInput8Create") == 0 && ModuleNameContains(hModule, "dinput8")) {
        ...
        static volatile LONG s_logged = 0;
        if (InterlockedCompareExchange(&s_logged, 1, 0) == 0)
            LogLine("GetProcAddress hook: DirectInput8Create wrapped");
...
static void LogExecutableFlavor()
{
    ...
    if (hasBind) {
        g_exeFlavor = 3;
        LogLine("executable flavor: packed SteamStub detected (entry RVA 0x%08X); fixed-RVA runtime voice patching will be used", (unsigned)ep);
    } else if (hasExtra) {
        g_exeFlavor = 2;
        LogLine("executable flavor: Steam unpacked detected (entry RVA 0x%08X)", (unsigned)ep);
    } else {
        g_exeFlavor = 1;
        LogLine("executable flavor: GOG/unpacked-like detected (entry RVA 0x%08X)", (unsigned)ep);
    }
}
...
static BYTE* ResolvePatchTarget(const char* name, DWORD gogRva, const BYTE* expected, DWORD expectedSize)
{
    ...
    if (scanned) {
        LogLine("%s: RVA mismatch, pattern found at RVA 0x%08X", name, (unsigned)(scanned - base));
        return scanned;
    }

    LogLine("%s: expected bytes not found", name);
    return NULL;
}
                                                ...
                                                bool allowFullScan)
{
    ...
    for (int i = 0; i < candidateCount; ++i) {
        ...
        if (rva + expectedSize <= imageSize) {
            ...
            if (MatchMaskAt(byRva, expected, mask, expectedSize)) {
                LogLine("%s: matched %s RVA 0x%08X", name, candidates[i].label, (unsigned)rva);
                return byRva;
            }
        }
    }
    ...
    if (allowFullScan) {
        ...
        if (scanned) {
            LogLine("%s: candidate mismatch, wildcard pattern found at RVA 0x%08X", name, (unsigned)(scanned - base));
            return scanned;
        }
    }

    LogLine("%s: expected bytes not found in supported static candidates", name);
    return NULL;
}
...
static bool WritePatch(const char* name, BYTE* dst, const BYTE* patch, DWORD patchSize)
{
    ...
    if (!VirtualProtect(dst, patchSize, PAGE_EXECUTE_READWRITE, &oldProtect)) {
        LogLine("%s: VirtualProtect failed, GetLastError=%lu", name, GetLastError());
        return false;
    }
    ...
    HMODULE mod = GetModuleHandleA(NULL);
    BYTE* base = (BYTE*)mod;
    LogLine("%s: patched at RVA 0x%08X", name, (unsigned)(dst - base));
    return true;
}
...
static bool ApplyPatchGroupAtomic(const char* groupName, PatchItem* items, int count)
{
    ...
    for (int i = 0; i < count; ++i) {
        ...
        if (!items[i].target) {
            LogLine("%s: atomic group skipped because %s did not match", groupName, items[i].name);
            return false;
        }
    }
    ...
    for (int i = 0; i < count; ++i) {
        if (!VirtualProtect(items[i].target, items[i].patchSize, PAGE_EXECUTE_READWRITE, &items[i].oldProtect)) {
            LogLine("%s: atomic group skipped because VirtualProtect failed on %s, GetLastError=%lu",
    ...
    for (int i = 0; i < count; ++i) {
        ...
        LogLine("%s: patched at RVA 0x%08X", items[i].name, (unsigned)(items[i].target - base));
    }

    LogLine("%s: atomic group applied", groupName);
    return true;
}
...
static bool TryApplyPackedSteamVoicePatch(const char* reason, bool logMiss)
{
    ...
    if (!voiceEnabled || !assetPatch || !requireAssetPatch) {
        if (logMiss)
            LogLine("Packed Steam voice runtime patch skipped by config: Enable=%d AssetPatch=%d RequireAssetPatch=%d",
                    voiceEnabled, assetPatch, requireAssetPatch);
    } else {
        if (logMiss)
            LogLine("Packed Steam voice runtime patch attempt: reason=%s", reason ? reason : "unknown");
        ...
        if (!ready) {
            if (logMiss)
                LogLine("Packed Steam voice runtime patch targets are not ready yet: reason=%s",
        ...
        if (ok) {
            ...
            LogLine("Packed Steam voice runtime patch applied: language=%d reason=%s",
                    voiceLang, reason ? reason : "unknown");
            WriteCompatibilityReport();
        } else if (logMiss) {
            LogLine("Packed Steam voice runtime patch not ready yet: reason=%s",
                    reason ? reason : "unknown");
        }
    }
...
static DWORD WINAPI PackedSteamVoicePatchThreadProc(LPVOID)
{
    ...
    LogLine("Packed Steam runtime patch thread started: tries=%d interval=%dms", tries, interval);

    for (int i = 1; i <= tries; ++i) {
        if (TryApplyPackedSteamVoicePatch("runtime-thread", i == 1 || (i % statusEvery) == 0)) {
            LogLine("Packed Steam runtime patch thread complete after %d pass(es)", i);
            return 0;
        }
        Sleep((DWORD)interval);
    }

    LogLine("Packed Steam runtime patch thread ended without applying voice patch");
    return 0;
}
...
static void StartPackedSteamVoicePatchThreadIfNeeded()
{
    ...
    if (h) CloseHandle(h);
        ...
        LogLine("Packed Steam runtime patch thread CreateThread failed, GetLastError=%lu", GetLastError());
    }
}
...
static void ApplyDpiAwareness()
{
    ...
    if (!enabled) {
        LogLine("DPI awareness disabled");
        return;
    }
    ...
    if (mode >= 3 && user32) {
        ...
        if (pSetProcessDpiAwarenessContext) {
            if (pSetProcessDpiAwarenessContext((HANDLE)(LONG_PTR)-4)) {
                LogLine("DPI awareness active: Per-Monitor V2");
                return;
            }
            LogLine("Per-Monitor V2 DPI failed, GetLastError=%lu", GetLastError());
        }
    }
    ...
    if (mode >= 2) {
        ...
        if (shcore) {
            ...
            if (pSetProcessDpiAwareness) {
                ...
                if (SUCCEEDED(hr)) {
                    LogLine("DPI awareness active: Per-Monitor");
                    return;
                }
                LogLine("Per-Monitor DPI failed, HRESULT=0x%08lX", (unsigned long)hr);
            }
        }
    }
    ...
    if (user32) {
        ...
        if (pSetProcessDPIAware) {
            if (pSetProcessDPIAware()) {
                LogLine("DPI awareness active: System");
                return;
            }
            LogLine("System DPI failed, GetLastError=%lu", GetLastError());
        }
    }

    LogLine("DPI awareness could not be set");
}
...
static void ApplyCpuAffinityLimit()
{
    ...
    if (!enabled) {
        LogLine("CPU affinity limiter disabled");
        return;
    }
    ...
    if (!GetProcessAffinityMask(proc, &processMask, &systemMask)) {
        LogLine("CPU affinity: GetProcessAffinityMask failed, GetLastError=%lu", GetLastError());
        return;
    }
    ...
    if (available <= 0) {
        LogLine("CPU affinity: no available CPU bits in affinity mask");
        return;
    }
    ...
    if (!newMask) {
        LogLine("CPU affinity: computed mask is zero, skipped");
        return;
    }
    ...
    if (newMask == processMask) {
        ...
        LogLine("CPU affinity already limited: max=%d available=%d mask=0x%p", maxCores, available, (void*)newMask);
        return;
    }
    ...
    if (SetProcessAffinityMask(proc, newMask)) {
        g_cpuAffinityAppliedCores = targetCores;
        g_cpuAffinityMask = newMask;
        LogLine("CPU affinity limited: requested=%d applied=%d available=%d mask=0x%p", maxCores, targetCores, available, (void*)newMask);
    } else {
        LogLine("CPU affinity: SetProcessAffinityMask failed, GetLastError=%lu requested=%d mask=0x%p", GetLastError(), maxCores, (void*)newMask);
    }
}
...
static void PatchIAT()
{
    ...
    LogLine("ADVAPI32 registry hooks installed: %d", advapiHooks);
    LogLine("KERNEL32 hooks installed: %d", kernelHooks);
    LogLine("DINPUT8 input hooks installed: %d", dinputHooks);
...
static int ReadWindowMode()
{
    // 0=fullscreen, 1=windowed, 2=borderless window. Default is intentionally borderless.
...
static void ApplyBorderlessToWindow(HWND hwnd)
{
    ...
    if (center) {
        ...
        if (GetMonitorInfoA(mon, &mi)) {
            ...
            LogLine("Borderless: GetMonitorInfo failed, GetLastError=%lu", GetLastError());
        }
    }
    ...
    LogLine("Borderless window applied: %d,%d %dx%d center=%d",
...
static DWORD WINAPI BorderlessThreadProc(LPVOID)
{
    ...
    LogLine("Borderless: main window not found within timeout");
    return 0;
}
...
static void StartBorderlessThreadIfNeeded()
{
    ...
    if (h) CloseHandle(h);
    else LogLine("Borderless: CreateThread failed, GetLastError=%lu", GetLastError());
}
...
static DWORD WINAPI FocusLossMouseReleaseThreadProc(LPVOID)
{
    ...
    for (;;) {
        ...
        if (unfocused) {
            ...
            ClipCursor(NULL);
            if (!wasUnfocused)
                LogTrace("Mouse released after game focus loss");
        }
...
static void StartFocusLossMouseReleaseThreadIfNeeded()
{
    ...
    if (h) {
        CloseHandle(h);
        LogLine("Focus-loss mouse release enabled for windowed/borderless mode");
    } else {
        InterlockedExchange(&g_focusReleaseThreadStarted, 0);
        LogWarn("Focus-loss mouse release thread failed, GetLastError=%lu", GetLastError());
    }
}
...
static void PatchGameMemory()
{
    ...
    LogExecutableFlavor();
    LogLine("TextLanguage=%d active via LNG_Language registry hook", textLang);
    LogLine("VoiceLanguage=%d", voiceLang);

    if (!voiceEnabled) {
        LogWarn("Voice patch disabled by config or because no requested/fallback voice package was found");
    } else if (IsPackedSteamStub()) {
        LogLine("Packed SteamStub detected: voice patch will wait for runtime unpacking and fixed Steam RVAs");
        ...
        LogLine("Voice FullPatch=1: Package + Resource + Bundle + Name will be patched transactionally");
        if (ApplyVoiceFullPatchWithCandidateSets("VoiceFullPatch",
                ...
                voiceLang, false)) {
            InterlockedExchange(&g_voiceFullPatchApplied, 1);
            LogLine("Voice package language=%d active", voiceLang);
            LogLine("Voice asset language=%d active", voiceLang);
        } else {
            LogLine("VoiceFullPatch failed; no partial voice patch was applied");
        }
    } else {
        LogLine("Voice patch warning: RequireAssetPatch=0 or AssetPatch=0 allows partial/legacy behavior");
        ...
        if (assetPatch) {
            ...
            if (ApplyPatchGroupAtomic("VoiceAssetPatch", items, 3)) {
                ...
                LogLine("VoiceAssetPatch failed as an atomic group; no partial asset patch was applied");
            }
        } else {
            LogLine("Voice AssetPatch disabled by [Voice] AssetPatch=0");
        }
    }
    ...
    if (windowMode >= 1) {
        ...
        if (PatchByRVAOrScan("WindowMode", 0x00929D9C,
            ...
            LogLine("WindowMode=%d active", windowMode);
        }
    } else {
        LogLine("WindowMode=0: fullscreen requested; window patch disabled");
    }
}
...
static void ValidateOneRange(const char* section, const char* key, int def, int lo, int hi)
{
    ...
    if (value != fixed) {
        ...
        LogWarn("Config corrected: [%s] %s=%d -> %d", section, key, value, fixed);
    }
}
...
static void ValidateAndCorrectConfig()
{
    ...
    if (aspect != fixedAspect) {
        ...
        ++g_configCorrectionCount;
        LogWarn("Config corrected: [Display] AspectRatio=%d -> %d", aspect, fixedAspect);
    }
    ...
    if (aa != fixedAa) {
        ...
        ++g_configCorrectionCount;
        LogWarn("Config corrected: [Graphics] AntiAliasing=%d -> %d", aa, fixedAa);
    }
...
static void CreateDefaultIni()
{
    ...
    wchar_t ini[32768];
#if defined(_MSC_VER)
    _snwprintf_s(ini, 32768, _TRUNCATE,
#else
    swprintf(ini, 32768,
#endif
        L"; ============================================================\r\n"
        L"; %ls v28 stable configuration\r\n"
        L"; This file is created as UTF-16 so non-ASCII paths and comments are safe.\r\n"
        L"; ASI module: %ls\r\n"
        L"; Generated names follow the ASI filename: %ls.ini, %ls.log,\r\n"
        L"; and %ls_CompatibilityReport.txt.\r\n"
        ...
        L"LogLevel=2\r\n"
        L"; Writes the ASI-named compatibility report with EXE, hooks, voice packs and settings.\r\n"
        ...
        g_asiBaseName, g_asiFileName, g_asiBaseName, g_asiBaseName, g_asiBaseName);
    ini[32767] = 0;
...
static void WriteCompatibilityReport()
{
    ...
    HANDLE f = CreateFileW(g_reportPath, GENERIC_WRITE, FILE_SHARE_READ, NULL,
                           CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
    if (f == INVALID_HANDLE_VALUE) {
        LogWarn("Compatibility report could not be created, GetLastError=%lu", GetLastError());
        return;
    }
    ...
    ReportLine(f, L"%s v28 compatibility report", g_asiBaseName);
    ReportLine(f, L"============================================================");
    ...
    ReportLine(f, L"ASI module: %s", g_asiPath);
    ReportLine(f, L"ASI directory: %s", g_asiDir);
    ReportLine(f, L"Config path: %s", g_iniPath);
    ReportLine(f, L"Log path: %s", g_logPath);
    ReportLine(f, L"Report path: %s", g_reportPath);
    ReportLine(f, L"");
    ...
    CloseHandle(f);
    LogLine("Compatibility report written");
}
...
BOOL WINAPI DllMain(HINSTANCE hInst, DWORD reason, LPVOID)
{
    if (reason == DLL_PROCESS_ATTACH) {
        ...
        InitializeAsiIdentityAndOutputPaths(hInst);
        ...
        if (g_logLevel > 0) {
            LogLine("loaded");
            char asiUtf8[MAX_PATH * 4] = {0};
            char iniUtf8[MAX_PATH * 4] = {0};
            char logUtf8[MAX_PATH * 4] = {0};
            char reportUtf8[MAX_PATH * 4] = {0};
            WideToUtf8(g_asiPath, asiUtf8, sizeof(asiUtf8));
            WideToUtf8(g_iniPath, iniUtf8, sizeof(iniUtf8));
            WideToUtf8(g_logPath, logUtf8, sizeof(logUtf8));
            WideToUtf8(g_reportPath, reportUtf8, sizeof(reportUtf8));
            LogLine("asi=%s", asiUtf8);
            LogLine("ini=%s", iniUtf8);
            LogLine("log=%s", logUtf8);
            LogLine("report=%s", reportUtf8);
        }
